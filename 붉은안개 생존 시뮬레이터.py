...
guards_db = {
    "헤일메리상": {"cost": 250, "power": 25, "dice": 10},
    "하융": {"cost": 275, "power": 0, "dice": 10},
    "죠니": {"cost": 300, "power": 30, "dice": 15},
    "서해수": {"cost": 400, "power": 60, "dice": 10}, 
    "암 이스엘": {"cost": 450, "power": 55, "dice": 15},
    ...
    "알토": {"cost": 600, "power": 85, "dice": 25},
    "아스트리트": {"cost": 600, "power": 82, "dice": 25},
...
items_db = {
    ...
    "영공명경전": {"cost": 200, "desc": "공격대가 절멸에 이르는 피해를 입을 시, 2회에 한해 붕괴를 막아냅니다."},
...
st.set_page_config(page_title="척추안개로부터 살아남기", layout="wide")
st.title("베르케칸 생존 시뮬레이터")
st.markdown("베르케 칸이 마침내 도래했습니다. 1턴부터요! 이거 진짜에요? 네 진짜에요!\n 다행히도 당연히 전력은 아닙니다. 심심해서 24시간 정도 외출 나오셨어요. 그것보다 더 나와계시면 훌라구가 측방을 타격할 거라 돌아가셔야 하고요.\n 그래도 패배하면 '취미생활' 용도로 쓰이는 건 똑같아요. 행운을 빌어요!")
...
if smoke_war_count >= 3:  # 발동 조건을 3인 이상으로 빡빡하게 올림!
    ...
    synergy_messages.append(f"💡 **[시너지 발견: 연기전쟁의 참상]** {smoke_war_count}인의 참전용사가 모여, 끔찍한 진흙탕 싸움을 유도합니다! (매 6시간마다 붉은안개의 위력 -{smoke_penalty})")
    ...
    if total_cost > BUDGET:
        ...
        for hour in range(1, target_hours + 1):
            ...
            if "푸른잔향 아르갈리아" in selected_guards and abs(effective_kali_attack - current_team_power) <= 20 and "푸른잔향 아르갈리아" not in missed_guards_this_turn:
                ...
                hour_log += "> 🎼 **[아르갈리아의 공명]** 칼리의 궤적과 아슬아슬하게 합을 맞추며 영구적인 흐름을 가져옵니다. (영구 방어선 +25)\n\n"
        ...
        if survival_status:
            ...
            st.error("❌ **미션 실패!** 공대원들은 전멸했고, 공대에 참여한 이들과 참여하지 않은 이들 중 일부가 머나먼 동쪽으로 사라졌습니다. 곧 다시 볼 수 있겠지요...")
